...
function App() {
  ...
  // 🔹 CREATE PEER CONNECTION
  async function createPeerConnection(socketId, userName, isOfferer) {
    const pc = new RTCPeerConnection(ICE_CONFIG);
    peersRef.current[socketId] = { pc, userName, streams: [] };
    ...
    pc.ontrack = (event) => {
      ...
      if (!peer.streams.find((s) => s.id === stream.id)) {
        peer.streams.push(stream);
      }
      setRemotePeers((prev) =>
        prev.some((p) => p.socketId === socketId)
          ? prev.map((p) =>
              p.socketId === socketId ? { ...p, streams: peer.streams } : p
            )
          : [...prev, { socketId, userName, streams: peer.streams }]
      );
    };
  ...
  async function handleOffer(fromSocketId, sdp, fromUserName) {
    const pc = new RTCPeerConnection(ICE_CONFIG);
    peersRef.current[fromSocketId] = { pc, userName: fromUserName, streams: [] };
        ...
        prev.some((p) => p.socketId === fromSocketId)
          ? prev.map((p) =>
              p.socketId === fromSocketId
                ? { ...p, streams: peer.streams }
                : p
            )
          : [...prev, { socketId: fromSocketId, userName: fromUserName, streams: peer.streams }]
      );
    };
  ...
  async function startScreenShare() {
    try {
      ...
      // Add screen track for all peers
  ...
  // 🔹 REMOTE VIDEO COMPONENT
  function RemoteVideo({ socketId, userName, streams }) {
    return (
      <div style={{ display: "inline-block", margin: 8 }}>
        <div style={{ textAlign: "center", marginBottom: 4 }}>{userName}</div>
        {streams.map((stream) => (
          <video
            key={stream.id}
            autoPlay
          ...
          <div style={{ display: "flex", gap: 20 }}>
            ...
            <div>
              ...
              {remotePeers.length === 0 && (
                <div style={{ color: "gray" }}>No one else in the room</div>
              )}
              {remotePeers.map((p) => (
                <RemoteVideo
                  key={p.socketId}
